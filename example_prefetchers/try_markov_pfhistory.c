  ...
  for(i=0; i<Page_Count; i++)
      ...
      for(j=0; j<64; j++)
	{
	  PF_pages[i].access_key1[j] = 0;
          ...
          PF_pages[i].delta_pointer[j] = 0;
	  PF_pages[i].pf_map[j] = 0;
	}
      printf("breakpoint2\n");
     int k;
     for(k=0; k<delta_table_Size; k++)
         {PF_pages[i].delta_delta[delta_table_Size]=0;
          PF_pages[i].delta_point[delta_table_Size]=0;
         } 
...
void l2_prefetcher_operate(int cpu_num, unsigned long long int addr, unsigned long long int ip, int cache_hit)
{
  ...
  if(page_index == -1)
      ...
      for(j=0; j<64; j++)
	{
	  PF_pages[page_index].access_key1[j] = 0;
          ...
          PF_pages[page_index].delta_pointer[j] = 0;
	  PF_pages[page_index].pf_map[j] = 0;
	}
     int k;
     for(k=0; k<delta_table_Size; k++)
         {PF_pages[page_index].delta_delta[k] = 0;
          PF_pages[page_index].delta_point[k] = 0;
         } 
 ...
 //       int count_prefetch;
        int trace_delta;
        int current_delta_pointer;
        current_delta_pointer = PF_pages[page_index].delta_pointer[access_index];
        unsigned long long int new_addr;
        int new_pageoffset;
  //      for(count_prefetch=0; count_prefetch <= Prefetch_Degree; count_prefetch++)
  //         {
            trace_delta = PF_pages[page_index].delta_delta[current_delta_pointer];
            //check the PF_Table 
            new_addr = addr + trace_delta;
            new_pageoffset =   (new_addr>>6) & 63;
            if(PF_pages[page_index].pf_map[new_pageoffset] == 1)
              {//do not prefetch
                 printf("Access key table: hit - Already Prefetched\n");
  //             count_prefetch--;
              }  
            else
              {
